/* =============================================================
   app.js — nav, reveals, wash drift, counters, sparklines
   ============================================================= */
(function () {
  'use strict';
  ...
  /* ---------- reveals ---------- */
  const revealEls = document.querySelectorAll('.reveal, .stmt__title, .band__title');
  ...
  document.querySelectorAll('.band__title').forEach((q) => {
    ...
    walk(q);
    q.querySelectorAll('.w').forEach((w, i) => { w.style.transitionDelay = (i * 45) + 'ms'; });
  });

  /* ---------- wash pointer drift ---------- */
  const wash = document.querySelector('.wash');
  if (wash && !reduced && matchMedia('(hover: hover)').matches) {
  ...
  /* ---------- deterministic sparklines (illustrative) ---------- */
  ...
  document.querySelectorAll('[data-spark]').forEach((c) => {
    ...
    window.addEventListener('resize', () => draw(p || 1));
  });
